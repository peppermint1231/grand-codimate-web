...
합성 자료에서 수정 전 사진 하단 915px/의견 상단 약 120px의 겹침을 재현했고, 수정 후 1440×1100·1280×800·390×844 모두 사진 하단 약 100px/의견 상단 약 120px로 겹침이 사라졌습니다. 긴 답변 끝까지 이동·접기·가로 넘침 없음·쓰기 없음 확인. 기록 `artifacts/scroll-before0106.json`, `artifacts/scroll-after0106.json`. 배포·운영·APK 검증은 `artifacts/deployment-0.10.6.json`에 기록합니다.

## 2026-09-23 사진별 의견 2열 구성 — 0.10.5

운영 완료: 웹 `6fffaaf5b1578887395b0ccd5f021658814185a4` GitHub 검사·Cloudflare 배포 성공. 기존 188개 자동 테스트 및 합성 화면의 데스크톱/모바일 2열·큰 사진·파일명 숨김·확대·번호 연동·사진별 저장/읽음 검증 통과. 운영 화면에서도 사진별 코멘트 카드의 큰 사진/오른쪽 입력칸·파일명 숨김과 상담상세 위치·모바일 표시를 확인했습니다. 운영 쓰기 0건, 검증 전후 전체 State 보존, 미전송 0건입니다.

APK 0.10.5/versionCode 31 기존 서명 유지, 내장/운영 파일 46개 및 공개 다운로드 SHA-256 `2dad5e9863462ba84c730f1216b27343324ec1e98d2625f17c4d9ae70cb085f0` 일치. 최종 기록 `artifacts/deployment-0.10.5.json`, `artifacts/photo-layout-production0105.json`. 검증용 Vite/Wrangler/Gradle은 종료했습니다.

[사용 안내](RELEASE-0.10.5.md). `OpinionPhotoLink`는 파일명 없이 현재 사진 번호/제외·삭제 상태만 표시하며 확대 제목도 동일합니다. 사진별 답변 카드는 왼쪽의 큰 정사각 사진(전체 맞춤)과 오른쪽 코멘트로 구성하고, 편집 카드도 입력칸/삭제 버튼을 오른쪽에 묶습니다. 모바일에서도 두 열을 유지하고 중첩 여백을 줄여 사진을 이전 64px보다 크게 표시합니다. 사진 ID/이름 스냅샷 등 저장 데이터·번호 계산·알림 동작은 변경하지 않았습니다.

기존 188개 자동 테스트 통과. `artifacts/photo-layout-browser0105.json`의 화면 검증은 데스크톱/모바일 2열·큰 사진·파일명 숨김·확대·사진별 저장/읽음/재배치 번호 연동을 포함합니다. 운영 읽기 전용 검증과 APK/자산/배포 결과는 `artifacts/deployment-0.10.5.json`에 기록합니다.

## 2026-09-23 상담 사진별 의사 코멘트 — 0.10.4

운영 완료: 웹 `11e7a97771a0fce362e0b846954ea6836a076054` GitHub 검사·Cloudflare 배포 성공. 188개 자동 테스트 및 합성 화면의 사진별 답변/알림/읽음/동적 번호/선택 제외/확대/직원 열람/주석 편집 중 초안 유지 검증 통과. 운영에서 사진 코멘트 선택 메뉴·상담탭 사진 아래 위치·기본 접힘·상담이력 목록에서 제거·모바일 화면을 읽기 전용으로 확인했습니다. 운영 쓰기 0건, 검증 전후 전체 State 보존, 미전송 0건입니다.

APK 0.10.4/versionCode 30 기존 서명 유지, 내장/운영 파일 46개 및 공개 다운로드 SHA-256 `a4c56cac2c2bf6f89c4ffa7e2a858326bd5003f15047eac15497b9bbded56ca0` 일치. 최종 기록 `artifacts/deployment-0.10.4.json`, `artifacts/photo-comments-production0104.json`. 로컬 검증용 Vite/Wrangler/Gradle은 종료했습니다.
...
운영 완료: 웹 `f368254fa660f6ea33fc8f25a8b77c0256ab9595` GitHub 검사·Cloudflare 배포 성공. 180개 자동 테스트 및 로컬 화면에서 개별/다중 삭제, Delete 입력 보호, undo/redo, 취소·재개·수정 이력, 의견 사진 주석/답변 저장과 상담 이동을 확인했습니다. 운영의 실제 의견 요청 사진으로 썸네일·확대·주석 편집기 진입·상담자/요청자 표시와 단가표 세 검색 모드·삭제 확인 취소·편집 취소/재개를 읽기 전용으로 검증했습니다. 실제 운영 삭제/저장 요청 0건, 전체 State 보존, 미전송 0건입니다.

APK 0.10.1/versionCode 27 기존 서명 유지, 내장/운영 파일 46개 및 공개 APK 다운로드 SHA-256 `25f272e259419172820a94b7cda7dd57e20476ad94941805c5a1d00079611f76` 일치. 최종 기록 `artifacts/deployment-0.10.1.json`, `artifacts/products-production0101.json`. 로컬 실행 중인 Vite/Wrangler/Gradle 작업은 종료했습니다.
