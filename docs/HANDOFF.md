...
## 2026-09-23 0.11.0 통합 개선

기기 우선 보류 상담 저장·자동 전송/충돌 복구, 변경 응답 반영과 단가표 원본/이력 지연 조회, 서버 환자 페이지/인덱스, 견적 디자인 2종, 전액 수납/환불, 직원·환자 상세 통계/Excel/인센티브/해석 프롬프트를 구현했다. 기존 대기 항목인 로그인 전 암호화 사진 복구, 메모 버전, 패키지·멤버십 안내 구조, 게시 전 비교/시험 장바구니, 관리자 인계/조회 감사, 큰 파일 이어 전송 및 원본 복구 분할 처리를 포함한다. 상세 사용법과 집계 기준은 [0.11.0 변경 기록](RELEASE-0.11.0.md), 작업 범위는 [계획](PLAN-0.11.md).

로컬 전체 257개 테스트와 빌드 통과. 브라우저 합성 검증은 기기 저장(서버 2.5초 지연 중 약 0.4~1.1초), 두 번 연속 변경, 재시작 후 큐, 충돌 보존, 별도 Excel/익명 프롬프트, 360~1920px 화면, 전액 수납·원수납별 환불, 두 테마 각 8장 JPG와 긴 PDF 포함. PDF 렌더에서 합계 배경과 이전 줄 겹침을 수정했다. 패키지에 실제 회차/금액 자동 차감은 없으며 설명과 상담 구성 스냅샷을 구조화한다. 초기 임상 상태 전체 내려받기는 유지하고 대용량 단가표 원문/수정이력을 분리했다.

기기 보관은 기존 선택형 암호화 저장을 사용하며, 앱이 종료/중단된 동안 전송을 보장하지 않는다. 금융 확정은 서버 확인 방식. 실제 미확정 가격/VAT/병원 지급률/동의서는 임의 확정하지 않았다. 물리 Android·카메라·펜·프린터 미연결로 실기기 검증 미완료. 게시 전 미리보기의 포털이 사이드바 뒤로 들어가던 문제도 실제 클릭 검증에서 발견해 overlay로 수정했다. 배포 검증 결과는 완료 후 아래에 추가한다.

## 2026-09-23 저장 진행 안내·고정 검색 — 0.10.12

운영 완료: 웹 `82ce661bd0235cd404a92a6a2c2d70934b8edbbc` GitHub 검사(232개 테스트)·Cloudflare 배포 성공. 운영 PC/태블릿/휴대폰에서 검색 고정·저장 안내와 기존 상담 팝업/견적서 진입 확인. 실제 자료 쓰기 0건, 전체 State 보존, 미전송 0건. APK 0.10.12/versionCode38 기존 서명 유지, 내장/운영 파일48개 및 공개 다운로드 SHA-256 `e569fa8fdd5b3ef4d7740f3d876e4dc05ef0a90464748f2d95defb0e4d25a649` 일치. 이전 APK에서 팝업 저장 후 body 스크롤 잠금을 재현하고 수정본 회귀 검증 통과. 검증용 Vite/Wrangler/Gradle 종료. Android 물리 기기 실행 검증은 미수행. 최종 기록 `artifacts/deployment-0.10.12.json`.
...
운영 완료: 웹 `1a9ed17c18d660b38056e638766412966e5cda1d` GitHub 검사(224개 테스트)·Cloudflare 배포 성공. 운영 PC/태블릿/모바일에서 상담 팝업·사진 높이·글자 크기·자동 메뉴 접기와 환자 견적서 서명 진입을 확인. 실제 환자 서명/QR 링크 생성 없이 쓰기 0건, 전체 State 보존, 미전송 0건. 합성 견적에서 PDF를 실제 렌더링해 한글/금액 확인, JPG 모든 페이지의 동의·서명과 인쇄/7일 QR/즉시 만료 확인. APK 0.10.10/versionCode36 기존 서명 유지, 내장/운영 파일48개와 공개 다운로드 SHA-256 `46ee5da56eff88782dbbf0cbf114eca9779f28704a9b63b6a534dc86c3234ead` 일치. Android 시스템 저장 플러그인 빌드/브리지 단위 테스트 완료; 물리 저장 창·프린터 검증은 미수행. 검증용 Vite/Wrangler/Gradle 종료. 최종 기록 `artifacts/deployment-0.10.10.json`. Android 소스는 기존 배포 방식대로 로컬 Git, 서명 APK는 공개 저장소에 보관.

[사용 안내](RELEASE-0.10.10.md). 사용자 추가 요청에 따라 0.10.9의 시술/장바구니 분할선과 사진 아래 메모/요청 배치를 대체했습니다. 오른쪽은 시술 선택 전용, 메모·의견 요청·장바구니는 하단 플로팅 버튼과 PhotoModal 팝업입니다. 닫기 시 입력 유지, 요청 성공 때만 요청 내용 초기화. 상단 바로가기 제거, 사진 헤더 한 줄, 상담 제목 옆 상태 배지, 환자 상세/상담에 성별·나이, 상담 상단 여백 축소. 상담 진입 시 사이드바 자동 접기와 240ms 애니메이션(동작 줄이기 존중), 모바일 하단 메뉴 유지.

사진뷰어 높이 480~1600px 드래그/터치/키보드 및 초기화·기기 저장. 사진 클릭 시 로컬 한 장 보기/썸네일 전환/전체 비교 복귀(선택 여부·저장 순서 보존). DisplaySettings의 UI 글자 크기 80~120%, 프리셋85/100/115, CSS 변수로 UI의 명시적 px 글자 크기를 조절합니다. 터치 입력칸은 최소16px, 문서/사진 주석 크기는 유지합니다.

Line.regularPrice에 선택 시 정가를 저장하고 price는 판매가를 유지합니다. calculate subtotal은 정가 합계, discountTotal은 상품 할인+추가 할인이며 공급가액/부가세는 기존 할인가 계산 유지. 신규/연장 서버 가격은 게시 SSOT에서 조회하며 클라이언트 금액을 무시합니다. 기존 저장 항목은 최신 정가를 소급하지 않습니다. 상품 할인만 있으면 별도 사유를 요구하지 않고 추가/확정 수정 사유는 유지합니다. 장바구니/견적/PDF/JPG에 정가·상품 할인·할인가와 추가 할인 내역 표시, JPG는 6개씩 페이지로 합계·동의 문구·서명 겹침 방지.

224개 자동 테스트, 합성 브라우저의 5개 화면 크기·팝업 입력 보존·사진 한 장 보기/크기 조절·글자 크기/설정 보존·PDF 텍스트/다중 JPG 확인. 운영·APK 최종 기록은 artifacts/deployment-0.10.10.json에 남깁니다. 실기기 실행 검증은 별도입니다.

병원용 PDF는 NanumGothic-Regular.ttf 전체 삽입(subset:false)으로 한글 누락 해결, pdftoppm 렌더링과 직접 화면 확인. 글꼴 소스 SHA는 scripts/annotation-fonts.json에 고정, OFL 동봉. saveDocuments는 웹 단일 다운로드/페이지별 재시도 및 Android ACTION_CREATE_DOCUMENT 또는 OPEN_DOCUMENT_TREE 저장(취소 처리, 오류 시 새 파일만 정리). 서명 APK 빌드와 브리지 단위 테스트 수행, 물리 기기 저장 창/프린터 실기 검증은 미수행.

PatientQuote: 동의/서명 → JPG/서명 이미지 인쇄/7일 QR. quoteConsents 새 State 섹션(기존 clinical signatures와 분리), quote.consent 서버 문구/버전 및 견적 해시 검증, audit.export quote-jpg에 현재 동의 필수. /api/quote-shares는 권한/동의 검증 후 256비트 토큰을 발급, 해시만 저장하고 JPEG 청크는 암호화. /api/public/quotes/:token 및 /:page는 유효기간/최신견적/환자상태/발급자 권한을 다시 확인. no-store/no-referrer/noindex/CSP, 즉시 만료/시간 정리/복구 시 전부 폐기. 공유 자료는 임시 SQL 별도 테이블이며 영구 OneDrive 백업은 동의 기록만 포함. 7일 기본값은 사용자가 확정함. 공유 링크 기능의 실제 환자 데이터 생성은 운영 검증에서 금지하며, 합성 브라우저와 메모리 SQLite Worker 테스트로 검증.

## 2026-09-23 상담 화면·터치 조작 — 0.10.9

운영 완료: 웹 `6e8005253bfcf1065bb971acd47bfa39d2f56d3b` GitHub 검사(202개 테스트)·Cloudflare 배포 성공. 운영에서 타임라인 페이지 제한, 사진 컬럼 내 메모·의견 요청, 시술 선택/장바구니 분할선과 PC·태블릿·모바일 배치를 확인했습니다. 운영 쓰기 0건, 전체 State 보존, 미전송 0건. APK 0.10.9/versionCode 35 기존 서명 유지, 내장/운영 파일 46개와 공개 다운로드 SHA-256 `37a5d77ad66aab6a527393fc5c65ccf7405711e801bc5c4076b2dc3acd0aebd4` 일치. 검증용 Vite/Wrangler/Gradle 종료. 최종 기록 `artifacts/deployment-0.10.9.json`.

[사용 안내](RELEASE-0.10.9.md). 환자 활동 타임라인은 20개 페이지와 제한 높이 내부 스크롤을 함께 적용합니다. 상담 메모·의견 요청을 사진 컬럼으로 이동하고 영역 바로가기를 추가했습니다. 시술 선택/장바구니는 독립 스크롤·높이 조절 막대·기기별 비율 저장·키보드/터치/초기화를 지원합니다. 휴대폰 하단 메뉴, 태블릿 축약 메뉴, 좁은 화면 세로 배치와 터치 영역을 조정했습니다.

PullToRefresh는 문서 맨 위의 단일 하향 터치만 처리하며 입력·사진·분할선·내부 스크롤 영역, 짧은/가로/취소 제스처를 제외합니다. 처리 중 또는 미전송 변경이 있으면 차단하고, refresh(true)로 작성 중인 상담을 보존합니다. 합성 Chromium에서 1440×1100·1024×768·768×1024·390×844·360×740 배치, 타임라인 마지막 페이지, 높이 조절, 터치 새로고침 성공/실패와 메모·장바구니·요청 보존을 확인합니다. 시술 목록 안의 이중 스크롤을 없애고 태블릿의 검색/제목 여백을 축소했습니다. 물리 기기 실행은 별도입니다. 운영·APK 최종 검증은 artifacts/deployment-0.10.9.json에 기록합니다.

## 2026-09-23 저장 상태·상담 판매·한국시간 — 0.10.8

운영 완료: 웹 `0039615044ce7dcf0ed66f83e59de522ed4e85f8` GitHub 검사(202개 테스트)·Cloudflare 배포 성공. 운영에서 상품 편집칸의 저장 상태, 상단 고정 안내 제거, 저장 버튼 비활성화, 한국시간 버전 표시, 일괄 상담 판매 선택, 모바일 배치를 확인했습니다. 운영 쓰기 0건, 전체 State 보존, 미전송 0건. 실제 상품은 자동 활성화·게시하지 않았습니다. APK 0.10.8/versionCode 34 기존 서명 유지, 내장/운영 파일 46개와 공개 다운로드 SHA-256 `1e5d08c4f8edc9c2fbc931ff9488cb5d713b8ab8c859fb84b05485b60ab3100e` 일치. 검증용 Vite/Wrangler/Gradle 종료. 최종 기록 `artifacts/deployment-0.10.8.json`.

[사용 안내](RELEASE-0.10.8.md). 기존 배너는 editable 여부만 검사해 저장 후에도 ‘저장하세요’가 남았습니다. `catalogHasChanges`로 현재/저장 초안 차이를 판별하고 상태·저장/게시 버튼을 오른쪽 상품 편집칸으로 이동(고정 해제)했습니다. 저장됨에는 실제 저장 시각, 게시됨에는 게시 시각/활성 수를 표시합니다. 미저장 변경은 게시 전 저장해야 하며 실패 시 초안을 유지합니다.

운영 진단 시 이벤트 게시본/초안 모두 182개·판매 활성 0개였고, 초안만 182개 검토완료였습니다(미전송 0). 실제 단가표 데이터는 자동 활성화/게시하지 않았습니다. 일괄 수정에 명시적 상담 판매 활성/비활성 선택 추가, 활성화 전 검토/가격/부가세 확인. 상담 빈 목록은 미게시/전체 판매 비활성/검색 불일치를 구분합니다. 기존 상담의 구버전은 `refreshConsultationBook`으로 해당 구분 장바구니가 비었을 때만 최신 게시본을 불러오며 다른 구분 금액·사진·메모는 보존합니다.

버전은 내부 식별자를 바꾸지 않고 초안 updatedAt/게시본 publishedAt을 Asia/Seoul로 표시해 UTC 노출과 복제 초안의 오래된 version 문자열 시각 문제를 해결했습니다. 이력 시간도 동일 함수 사용. 202개 테스트 및 합성 화면 검증, 운영 읽기 전용 결과는 `artifacts/deployment-0.10.8.json`에 기록합니다.

## 2026-09-23 선택 상품 일괄 수정 — 0.10.7

운영 완료: 웹 `71de3ee40b1c74cd2abb2efb1116b38147e10dcd` GitHub 검사(197개 테스트)·Cloudflare 배포 성공. 운영 화면에서 일괄 수정 선택/적용 활성화·취소·이벤트 정가/이벤트가 입력·삭제 확인 기본 초점/Esc·모바일 배치를 확인했습니다. 운영 쓰기 0건, 전체 State 보존, 미전송 0건. APK 0.10.7/versionCode 33 기존 서명 유지, 내장/운영 파일 46개와 공개 다운로드 SHA-256 `0a554a18d259d896db1d6ba18a2431f783b2624d1b5c09961658da0248f1fe05` 일치. 검증용 Vite/Wrangler/Gradle 종료. 최종 기록 `artifacts/deployment-0.10.7.json`.
